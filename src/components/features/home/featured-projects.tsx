...
export async function FeaturedProjects() {
  ...
  const shown = (projects && projects.length > 0) ? projects : LOCAL_PROJECTS.slice(0, 3)
    ...
    <section className="py-24 bg-black/20">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {(shown as (Project & { profiles?: { full_name?: string } })[]).map((project) => (
            <Link key={project.id} href={`/projects/${project.slug}`} className="group block">
              <div className="relative rounded-2xl overflow-hidden bg-white/5 border border-white/10 hover:border-primary/50 transition-all hover:-translate-y-1"/>
